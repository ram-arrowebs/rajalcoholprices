...
3. **Loading:** the inline script fetches `./data/prices.json` with `cache: "no-cache"` and maps each object to `[BrandName, Type, Packing+"ml", MRP]` for `feedDataTable`. `./data/meta.json` is fetched separately to fill `#lastupdate`; its absence is tolerated.
4. **Presentation:** DataTables (Bootstrap 5 theme) renders the table. Column `render` callbacks handle display formatting (proper-casing item names, `en-IN` currency formatting of price for `display` type only so sorting stays numeric). Sort arrows are hidden via CSS in `styles.css`.
...
- The last two commits are "type filter - in progress": `fnInitComplete` builds a per-column `<select>` filter and appends it to `column.footer()`, but the table has no `<tfoot>`, so the selects have nowhere to render. Finishing this feature requires adding a footer row (or moving the selects to the header) and removing the debug `console.log` calls.
