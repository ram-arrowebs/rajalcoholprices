...
3. **Loading:** the inline script fetches `./data/prices.json` with `cache: "no-cache"` and maps each object to `[BrandName, Packing+"ml", MRP]` (the source `Type` field is not shown) for `feedDataTable`. `./data/meta.json` is fetched separately to fill `#lastupdate`; its absence is tolerated.
...
- `fnInitComplete` still contains a leftover per-column `<select>` filter loop (from an abandoned "type filter" attempt) that appends to `column.footer()`. The table has no `<tfoot>`, so it renders nothing; it can be removed or given a footer row if a filter is wanted.
